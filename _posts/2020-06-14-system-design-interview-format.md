---
layout: post
title: "System Design Interview Format - 6 Steps to passing"
...
100 Million requests per month = ~40 Queries per second (QPS)
Number of corse = QPS * Request time (100ms)

For example, lets estimate the number of machines we need to support 1 Billion users making 20 requests per day.

50 million users * 20 requests per day * 31 = 620 million requests per month.
62 * 40 QPS = ~240 QPS # just memorize that 10m request per mo = ~40 qps
240 * 0.1s per request = 24 cores
